...
func voteProcessing(forIssue: String, withVotes: [Bool]){
    var yes = 0
    var no = 0
    for result in withVotes{
        if result{
            yes += 1
        } else{
            no += 1
...
voteProcessing(forIssue: "Should we change the mascot?", withVotes: shouldMascotChangeVotes)
